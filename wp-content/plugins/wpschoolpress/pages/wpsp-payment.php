...
foreach($wpsp_clas_data as $clsloop){
        ...
        $wpsp_order_data =$wpdb->get_results("SELECT c.cid,f.class_id,f.fees_amount,f.created_date FROM  wp_wpsp_class as c INNER JOIN wp_wpsp_fees AS f ON f.class_id = c.cid where f.class_id = '".$clsloop->cid."'");
...
?>
                                    <tr>
                                        ...
                                        <td><?php if($newDate != ''){ echo $newDate; }else { echo '-'; }?></td>
                                        <td><?php echo $paid;?></td>
...
$wpsp_teacher_data =$wpdb->get_results("SELECT * FROM  $cteacher
 where wp_usr_id = '".$clsloop->teacher_id."'");
        ...
        // echo"<pre>";print_r($courses_check);
         $wpsp_stud =$wpdb->get_results("SELECT * FROM  $stable s  where  s.parent_wp_usr_id = '".$current_user->ID."' ORDER BY s.wp_usr_id DESC");
        ...
        foreach($wpsp_stud as $value){
            ...
            $wpsp_cls_data =$wpdb->get_results("SELECT s.order_id,s.fees_amount,s.created_date,s.student_id,s.class_id,c.cid,c.c_name,c.teacher_id  FROM  wp_wpsp_fees AS s INNER JOIN wp_wpsp_class AS c ON c.cid=s.class_id where student_id = '".$value->wp_usr_id."' ORDER BY s.student_id DESC");
    ...
    ?>
                                    <tr>
                                        <td><?php echo  $value->s_fname.' '.$value->s_lname;?></td>
                                        <td><?php echo  $value->s_rollno;?></td>
                                        <td><?php echo  $wpsp_cls_data[0]->c_name?></td>
                                        <td> <?php echo  $wpsp_teacher_data[0]->first_name.' '.$wpsp_teacher_data[0]->last_name;?></td>

                                        <td><?php
                                        $price = $wpsp_cls_data[0]->fees_amount;
                                        if($price != ''){ echo get_woocommerce_currency_symbol().$price .'.00'; }else { echo '-'; }?>
                                        </td>
                                        <td>
                                        <?php
                                        // ->date('Y F j, g:i A');
                                        $order_date_created = $wpsp_cls_data[0]->created_date;
